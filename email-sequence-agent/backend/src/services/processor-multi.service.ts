import { Lead, ResearchResult, EmailSequence, SpreadsheetConfig } from '../types.js';
...
export async function processAllResearch(): Promise<{ processed: number; errors: string[] }> {
  const errors: string[] = [];
  try {
...
export async function processAllSequenceGeneration(): Promise<number> {
  try {
...
export async function processAllSending(): Promise<number> {
  try {
