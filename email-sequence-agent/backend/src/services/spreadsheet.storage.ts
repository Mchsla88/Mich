...
export async function loadSpreadsheets(): Promise<SpreadsheetConfig[]> {
  try {
    await ensureDataDir();
...
export async function saveSpreadsheets(spreadsheets: SpreadsheetConfig[]): Promise<void> {
  try {
    await ensureDataDir();
...
export async function getActiveSpreadsheets(): Promise<SpreadsheetConfig[]> {
  const all = await loadSpreadsheets();
  return all.filter(s => s.active);
}

// Get specific spreadsheet by ID
export async function getSpreadsheetById(id: string): Promise<SpreadsheetConfig | null> {
  const all = await loadSpreadsheets();
...
export async function addSpreadsheet(
  spreadsheet: Omit<SpreadsheetConfig, 'id' | 'createdAt' | 'updatedAt'>
...
export async function updateSpreadsheet(
  id: string,
  updates: Partial<Omit<SpreadsheetConfig, 'id' | 'createdAt' | 'updatedAt'>>
...
export async function toggleSpreadsheetActive(id: string): Promise<SpreadsheetConfig | null> {
